...
<div id="blog-listing" class="blogs-container">
 	{if isset($filter.type)}
	 	{if $filter.type=='tag'}
			<h1 class="blog-title">{l s='Filter Blogs By Tag' mod='leoblog'} : <span>{$filter.tag}</span></h1>
		{elseif $filter.type=='author'}
			<h1 class="blog-title">{l s='Filter Blogs By Blogger' mod='leoblog'} : <span>{$filter.employee->firstname} {$filter.employee->lastname}</span></h1>
		{/if}
	{else}
	<h1 class="blog-title">{l s='Lastest Blogs' mod='leoblog'}</h1>
	{/if}
